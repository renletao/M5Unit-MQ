...
void M5UnitMQ::readBytes(uint8_t addr, uint8_t reg, uint8_t *buffer,
                         uint8_t length) {
  uint8_t index = 0;
  _wire->beginTransmission(addr);
  _wire->write(reg);
...
void M5UnitMQ::releaseMutex() { mutexLocked = false; }

bool M5UnitMQ::begin(TwoWire *wire, uint8_t addr, uint8_t sda, uint8_t scl,
                     uint32_t speed) {
  _wire = wire;
  _addr = addr;
  _sda = sda;
...
void M5UnitMQ::setPulseLevelTime(uint8_t highLevelTime, uint8_t lowLevelTime) {
  ...
  uint8_t reg = UNIT_MQ_HEATING_HIGH_PLUSE_CFG_REG_ADDR;
...
uint16_t M5UnitMQ::getReferenceVoltage(void) {
  acquireMutex();
  uint16_t tempValue = 0;
  uint8_t reg = INTERNALT_REFERENCE_VOLTAGE_REG_ADDR;
  readBytes(_addr, reg, (uint8_t *)&tempValue, 2);
  releaseMutex();
  return tempValue;
}
...
uint16_t M5UnitMQ::getMQChannelVoltage(void) {
  acquireMutex();
  uint16_t tempValue = 0;
  uint8_t reg = MQ_CHANNEL_VOLTAGE_REG_ADDR;
  readBytes(_addr, reg, (uint8_t *)&tempValue, 2);
  releaseMutex();
  return tempValue;
}
uint16_t M5UnitMQ::getNTCChannelVoltage(void) {
  ...
  acquireMutex();
  uint16_t tempValue = 0;
  uint8_t reg = NTC_CHANNEL_VOLTAGE_REG_ADDR;
  readBytes(_addr, reg, (uint8_t *)&tempValue, 2);
  releaseMutex();
  return tempValue;
}
